...
import { cachedQuery, qrm } from 'lib/db';
...
class GuildsDB {
        ...
        return this.dbGetBySlug(guildSlug)
            .then((guild) => {
                if(guild !== null) {
                    ...
                    return __getRemote({guild_name: slugifier.deslugify(guildSlug)});
                }
            })
    }
    ...
    */
    ...
    __getRemote(params, key, fnInProgress, fnCallback) {
        ...
        return fetch(`https://api.guildwars2.com/v1/guild_details`, params)
            .then((response) => {

                const guildData = response.json();

                if (response.status !== 200) {
                    throw(res.statusText);
                }
                else if (!guildData || !guildData.hasOwnProperty('guild_id')) {
                    throw('NotFound');
                }
                else {
                    return guildData;
