...
function __loadIndex(indexPath) {
    ...
    try {
        let _fromDisk = fs.readFileSync(indexPath);
        guildsIndex = JSON.parse(_fromDisk);
...
function __writeIndexBase(indexPath, guildData){
    ...
    let _toDisk = JSON.stringify(guildData.toJS());


    try {
        console.log('__writeIndexBase()', indexPath, _toDisk.length);
        fs.writeFile(indexPath, _toDisk, function(err){
            console.log('__writeIndexBase() completed', 'ERR', err);
        });
    }
    catch(err) {
        console.log('ERROR:', err);
