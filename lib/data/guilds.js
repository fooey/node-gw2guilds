...
class GuildsDB {
    ...
    init(fnCallback) {
        ...
        fs.mkdirs(this.dataPath, err => {
            console.log('DB::guilds::mkdirs', this.dataPath);

            if (err) {
                console.log('DB::guilds::mkdirs:ERR', err);
            }
...
function __loadIndex(indexPath, fnCallback) {
    ...
    fs.readFile(indexPath, (err, data) => {
        ...
        if (guildsIndex.isEmpty()) {
            setTimeout(__writeIndexBase.bind(null, indexPath, guildsIndex), 10);
        }

        fnCallback(err, guildsIndex);
    });
}
