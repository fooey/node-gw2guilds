...
function getBackground(id, colorId, flags, size) {
    ...
    if (!bg || !bg.p || !bg.p.length) {
        return '';
    }

    const bgColor   = getRGB(colorId, [0, 0, 0]);
...
function getForeground(emblemData, size) {
    ...
    const color1    = getRGB(emblemData.foreground_secondary_color_id, [255, 255, 255]);    // getColorHex(emblemData.foreground_secondary_color_id) || '#FFFFFF';
    const color     = getRGB(emblemData.foreground_primary_color_id, [255, 0, 0]);          // getColorHex(emblemData.foreground_primary_color_id) || '#FF0000';
    const transform = getTransformAttrib('fg', emblemData.flags, size);

    // if (fg.pto2 && fg.pto2.length) {
    //     console.log('pto2', fg.pto2);
    //     console.log('emblem', emblemData);
    // }
...
function drawShapes(shapes, fill, opacity) {
    ...
    if (shapes && Array.isArray(shapes) && shapes.length) {
...
function getRGB(colorId, fallback) {
    const rgb = colors.getRGBById(colorId) || fallback;
    return `rgb(${rgb.join()})`;
}
